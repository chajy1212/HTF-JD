...
# ============================================================
# ============================================================
# 2) CT-level Self-Attention MIL classifier
# ============================================================
class SelfAttentionMIL(nn.Module):
    def __init__(self, feat_dim=128, num_heads=4, num_classes=2):
        super().__init__()
        self.encoder = PatchEncoder(feat_dim)

        # 🔥 Multi-head Self-Attention (Transformer 스타일)
        # batch_first=True 이니까 입력/출력 shape: (batch, seq, feat_dim)
    ...
    def forward(self, bag, batch_size=64):
        """
        bag : (N, 1, 64, 64)  - 하나의 CT에 대한 모든 patch
        return:
          logits:    (1, num_classes)       - bag-level prediction
          feats_sa:  (N, feat_dim)          - self-attention 거친 patch feature
          bag_feat:  (feat_dim,)            - pooled bag feature
          attn_mat:  (N, N) or (num_heads, N, N) - patch 간 self-attention map
        """
        all_feats = []
        N = bag.size(0)

        # 1) patch encoder (chunk 단위로 처리해서 메모리 절약)
        ...
        # 2) Self-Attention: patch들끼리 서로 Q/K/V attention
        #   - self_attn 입력: (batch=1, seq=N, feat_dim)
        feats_in = feats.unsqueeze(0)           # (1, N, feat_dim)
        feats_sa, attn_weights = self.self_attn(
            feats_in,  # query
            feats_in,  # key
            feats_in   # value
        )
        feats_sa = feats_sa.squeeze(0)          # (N, feat_dim)

        # 3) bag-level feature (여기는 mean pooling, max로 바꿔도 됨)
        bag_feat = feats_sa.mean(dim=0)         # (feat_dim,)

        # 4) CT-level logit
        logits = self.classifier(bag_feat.unsqueeze(0))  # (1, num_classes)

        # attn_weights shape:
        #   average_attn_weights=True(default)이면 (1, N, N) (batch, tgt_len, src_len)
        #   여기서는 분석용으로 그대로 반환해둘게
        attn_mat = attn_weights.squeeze(0)      # (N, N) 정도 될 것

        return logits, feats_sa, bag_feat, attn_mat
...
def train_model():
    ...
    # 2단계: (Val+Test) → Val / Test  (각각 전체의 25%씩 → temp의 50%/50%)
    ...
    # Model
    model = SelfAttentionMIL().to(device)
    ...
    best_model_path = "/home/daeun/Workspace/HTF-JD/model_SelfAttention_best.pth"
