...
class MILClassifier(nn.Module):
    def __init__(self, feat_dim=128, num_classes=2):
        ...
        # 🔥 Attention 네트워크 추가 (Ilse et al., 2018 스타일)
        self.attn = nn.Sequential(
            nn.Linear(feat_dim, 128),
            nn.Tanh(),
            nn.Linear(128, 1)      # → 각 patch마다 scalar score
        )
    ...
    def forward(self, bag, batch_size=64):
        ...
        attn_raw = self.attn(feats)            # (N, 1)
        attn_scores = torch.softmax(attn_raw, dim=0)  # (N, 1), 합=1

        # 3) 가중합으로 bag feature 만들기
        bag_feat = torch.sum(attn_scores * feats, dim=0)   # (feat_dim,)

        # 4) CT-level logits
        logits = self.classifier(bag_feat.unsqueeze(0))    # (1, num_classes)

        # logits, patch feature, bag feature, attention weight 반환
...
def train_model():
    ...
    best_model_path = "/home/daeun/Workspace/HTF-JD/model_Attention_best.pth"
